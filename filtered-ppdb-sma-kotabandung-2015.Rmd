...
plot_pg <- function(option_id, name) {
  ...
  plot <- ggplot(df, aes(x=snapshottime, y=pg_dw)) +
    ...
    scale_x_datetime(name='hijau=dalam wilayah | kuning=luar wilayah | merah=luar kota') +
    geom_errorbar(position=position_jitter(height=0), color='green',
                 aes(ymin=low_dw, ymax=high_dw)) +
    geom_line(aes(y=pg_dw), color='green', size=1.5, alpha=0.5, position=position_jitter(height=1)) +
    geom_line(aes(y=mean_dw), color='green', linetype='dashed') +
    geom_errorbar(position=position_jitter(height=0), color='yellow',
                 aes(ymin=low_lw, ymax=high_lw)) +
    geom_line(aes(y=pg_lw), color='yellow', size=1.5, alpha=0.5, position=position_jitter(height=1)) +
    geom_line(aes(y=mean_lw), color='yellow', linetype='dashed') +
    geom_errorbar(position=position_jitter(height=0), color='red',
                 aes(ymin=low_lk, ymax=high_lk)) +
    geom_line(aes(y=pg_lk), color='red', size=1.5, alpha=0.5, position=position_jitter(height=1)) +
